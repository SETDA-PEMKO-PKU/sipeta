...
class OpdJabatanController extends Controller
{
    ...
    public function create()
    {
        ...
        // Auto-fill parent from query string (e.g. from peta jabatan "tambah anak" button)
        $defaultParentId = request()->get('parent_id');
        $defaultParent = $defaultParentId
            ? Jabatan::find($defaultParentId)
            : null;

        // Nama jabatan referensi grouped by jenis untuk dropdown
        $namaReferensi = \App\Models\NamaJabatanReferensi::orderBy('nama')
            ->get()
            ->groupBy('jenis_jabatan')
            ->map(fn($items) => $items->map(fn($r) => ['nama' => $r->nama])->values());

        $jenisOptions = \App\Models\NamaJabatanReferensi::jenisOptions();

        return view('admin.jabatan.create', compact(
            'opds', 'parentJabatans', 'defaultParent', 'defaultParentId',
            'namaReferensi', 'jenisOptions'
        ));
    }
